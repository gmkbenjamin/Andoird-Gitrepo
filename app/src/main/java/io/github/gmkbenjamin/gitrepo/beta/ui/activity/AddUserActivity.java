...
public class AddUserActivity extends BaseActivity {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        //remove select from contact to avoid privacy issues
/*
    ...
    private void processUserAction() {
        ...
        if (editMode) {
            try {
                ...
                if (publickeyEditText.getText().toString().trim().length() > 0) {
                    checkUser = getHelper().getUserDao().queryForPublickey(publickeyEditText.getText().toString().trim());
                    if (publickeyEditText.getText().toString().trim() != "" && checkUser != null && checkUser.getId() != userId) {
        ...
        if (requestCode == CONTACT_PICKER) {
            ...
            if (c.moveToFirst()) {
                ...
                if (name != null && !"".equals(name.trim())) {
                    ...
                    usernameEditText.setText(GitrepoCommons.toCamelCase(firstWord));
                }
